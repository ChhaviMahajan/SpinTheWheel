...
class AnimationUtility: NSObject {
    ...
    class func group(animations : [CAAnimation], fillMode : String!, forEffectLayer : Bool = false, sublayersCount : NSInteger = 0) -> CAAnimationGroup!{
        ...
        if forEffectLayer{
            groupAnimation.duration = AnimationUtility.maxDuration(ofEffectAnimation: groupAnimation, sublayersCount: sublayersCount)
        }else{
            groupAnimation.duration = AnimationUtility.maxDuration(ofAnimations: animations)
        }
